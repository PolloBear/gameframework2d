...
typedef struct
{
	...
	GFC_Vector2D	size;  //width and heigt of the screen
	GFC_Rect		bounds;
	Bool			bindCamera;
...
{
	_camera.position.x = target.x - (_camera.size.x * 0.5);
	_camera.position.y = target.y - (_camera.size.y * 0.5);

	// Apply bounds if enabled to stop at map edges
	if (_camera.bindCamera) camera_apply_bounds();
